function SaveNormPerPerson(AllSubjData,SeparateNost,WSA)
...
if all([strcmpi('wake',WSA)])
    for i=1:size(AllSubjData,2)
        ...
        NormDataToUse(:,[2,4]) = zscore(NormDataToUse(:,[2,4]));
        ...
        if ~exist(['Data/' subjData.Name '/norm_full_data_wake_5min_block.mat'],'file')
            if ~exist(['Data/' subjData.Name],'dir')
                mkdir(['Data/' subjData.Name])
                save(['Data/' subjData.Name '/norm_full_data_wake_5min_block.mat'],"labels","raw_in_block",'group_labels');
            else
                save(['Data/' subjData.Name '/norm_full_data_wake_5min_block.mat'],"labels","raw_in_block",'group_labels');
            end
        end
...
elseif  all([strcmpi('sleep',WSA)])
    for i=1:size(AllSubjData,2)
        ...
        if ~exist(['Data/' subjData.Name '/norm_full_data_sleep_5min_block.mat'],'file')
            if ~exist(['Data/' subjData.Name],'dir')
                mkdir(['Data/' subjData.Name])
                save(['Data/' subjData.Name '/norm_full_data_sleep_5min_block.mat'],"labels","raw_in_block",'group_labels');
            else
                save(['Data/' subjData.Name '/norm_full_data_sleep_5min_block.mat'],"labels","raw_in_block",'group_labels');
            end
        end
