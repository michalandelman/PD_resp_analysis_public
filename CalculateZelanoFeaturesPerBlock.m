function CalculateZelanoFeaturesPerBlock(AllSubjData,WSA)

zscored=true;
for sbj=1:size(AllSubjData,2)
    SubjectName=AllSubjData(sbj).Name;
    if all(strcmpi('wake',WSA))
        file=dir(['/Users/michalag/Desktop/Michal/University/PhD/Research_Subjects/Olfactory_Parkinson/Nasal_Cycle/nasal-cycle-pd/code/features_extraction/Data/' SubjectName '/norm_full_data_wake_1min_block.mat']);
    elseif  all(strcmpi('sleep',WSA))
        file=dir(['/Users/michalag/Desktop/Michal/University/PhD/Research_Subjects/Olfactory_Parkinson/Nasal_Cycle/nasal-cycle-pd/code/features_extraction/Data/' SubjectName '/norm_full_data_sleep_1min_block.mat']);
    end
    ...
    if ~exist(['Data/' SubjectName '/' SubjectName '_zelano_' WSA '_5min_no_overlap_normalized.mat'],'file')
        if zscored
            save(['Data/' SubjectName '/' SubjectName '_zelano_' WSA '_5min_no_overlap_normalized.mat'],'mat','labels');
        else
            save(['Data/' SubjectName '/' SubjectName '_zelano_' WSA '_5min_no_overlap.mat'],'mat','labels');
        end
    end
